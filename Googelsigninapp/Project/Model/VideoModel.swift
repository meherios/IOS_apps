...
class VideoModel: NSObject {
    ...
    //let API_KEY = "AIzaSyCqyUQnsG9u-qyw_-G-KqFdPTI_GRQ-cx0"
    ...
    func getFeedVideos(){
        ...
        Alamofire.request("https://www.googleapis.com/youtube/v3/search", method: HTTPMethod.get, parameters: ["part":"snippet", "key": API_KEY, "type": "", "maxResults": 20], encoding: URLEncoding.default, headers: nil).responseJSON { (response) in
            print(response)
            if let JSON = response.result.value {
                if let dict = JSON as? [String: Any]{
                    ...
                    for video in dict["items"] as! [AnyObject] {
                        ...
                        videoObj.videoTitle = video.value(forKeyPath: "snippet.channelTitle") as! String
